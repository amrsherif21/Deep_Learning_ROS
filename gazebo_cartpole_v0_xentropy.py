...
if __name__ == '__main__':
    ...
    # Determine the size of the observation space (input layer size) and the number of possible actions (output layer size)
    obs_size = env.observation_space.shape[0]
    n_actions = env.action_space.n

    # Create the neural network with the specified input, hidden, and output sizes
    ...
    # Tensorboard writer for tracking progress
    writer = SummaryWriter(comment="-cartpole")

    # Training loop over batches of episode data
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        # Filter the batch to obtain the top-performing episodes based on the reward percentile
        obs_v, acts_v, reward_b, reward_m = filter_batch(batch, PERCENTILE)

        # Zero the gradients before performing a forward pass and backpropagation
        optimizer.zero_grad()

        # Perform the forward pass to predict action probabilities
        action_scores_v = net(obs_v)

        # Calculate the loss between predicted action probabilities and the actual actions taken
        loss_v = objective(action_scores_v, acts_v)

        # Perform backpropagation to compute gradients
        loss_v.backward()

        # Update the weights of the neural network using the computed gradients
        optimizer.step()

        # Print batch summary and record progress using Tensorboard
        ...
        # Check if the mean reward has surpassed the threshold to consider the problem solved
